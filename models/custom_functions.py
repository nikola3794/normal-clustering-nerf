...
class VolumeRenderer(torch.autograd.Function):
    ...
    @staticmethod
    @custom_fwd(cast_inputs=torch.float32)
    def forward(ctx, sigmas, raws, deltas, ts, rays_a, T_threshold):
        total_samples, opacity, depth, rend, ws = \
            vren.composite_train_multi_fw(sigmas, raws, deltas, ts,
                                    rays_a, T_threshold)
        ctx.save_for_backward(sigmas, raws, deltas, ts, rays_a,
                              opacity, depth, rend, ws)
        ctx.T_threshold = T_threshold
        return total_samples.sum(), opacity, depth, rend, ws

    @staticmethod
    @custom_bwd
    def backward(ctx, dL_dtotal_samples, dL_dopacity, dL_ddepth, dL_drend, dL_dws):
        sigmas, raws, deltas, ts, rays_a, \
        opacity, depth, rend, ws = ctx.saved_tensors
        dL_dsigmas, dL_draws = \
            vren.composite_train_multi_bw(dL_dopacity, dL_ddepth, dL_drend, dL_dws,
                                    sigmas, raws, ws, deltas, ts,
                                    rays_a,
                                    opacity, depth, rend,
                                    ctx.T_threshold)
        return dL_dsigmas, dL_draws, None, None, None, None
