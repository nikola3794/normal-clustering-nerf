...
class NGPMT(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        hidden_layer_rgb = 2
        hidden_layer_sigma = 1
        print(f"hidden_layer_rgb:{hidden_layer_rgb}")
        print(f"hidden_layer_sigma:{hidden_layer_sigma}")
        ...
        self.sigma_net = tcnn.Network(
            ...
            network_config={
                ...
                "output_activation": "None",
                "n_neurons": 64,
                "n_hidden_layers": hidden_layer_sigma,
            }
        )
        ...
        self.rgb_net = \
            tcnn.Network(
                n_input_dims=19, n_output_dims=3,
                network_config={
                    "otype": "FullyFusedMLP",
                    "activation": "ReLU",
                    "output_activation": self.rgb_act,
                    "n_neurons": 64,
                    "n_hidden_layers": hidden_layer_rgb,
                }
            )
        ...
        if self.pred_sem:
            self.sem_net = \
                tcnn.Network(
                    n_input_dims=16, n_output_dims=kwargs['n_sem_cls'],
                    network_config={
                        "otype": "FullyFusedMLP",
                        "activation": "ReLU",
                        "output_activation": 'None',
    ...
    def density(self, x, return_feat=False):
        ...
        h = self.sigma_net(self.xyz_encoder(x))
    ...
    @torch.no_grad()
    def mark_invisible_cells(self, K, dev, poses, img_wh, near_distance, chunk=64**3):
        """
        ...
        if isinstance(K, torch.Tensor):
            ...
            M_ndc_from_cam = K[0].to(dev)
            M_uv_from_ndc = K[1].to(dev)
            shift = torch.tensor(K[2]).to(dev)
            scale = K[3]
        else:
        ...
        for c in range(self.cascades):
            ...
            for i in range(0, len(indices), chunk):
                ...
                if isinstance(K, torch.Tensor):
                    ...
                    # Convert back to original scale for M to work
                    xyzs_c *= 2 * scale
                    xyzs_c = torch.cat((xyzs_c, torch.ones_like(xyzs_c)[:, :1, :]), 1)
                    xyz_clip = M_ndc_from_cam @ xyzs_c
                    xyz_ndc = xyz_clip / xyz_clip[:, 3:]
                    uvd = M_uv_from_ndc @ xyz_ndc
                    uv = uvd[:, :2]
                # TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO TODO 
                in_image = (uvd[:, 2]>=0)& \
        ...
        # (self.density_grid == 0).sum()/(self.density_grid.shape[1])
