...
////////////////////////////////////////////////////////////////////////////////
// I added:
// composite_train_multi_fw()
// composite_train_multi_bw()
// composite_test_multi_fw()
////////////////////////////////////////////////////////////////////////////////
...
std::vector<torch::Tensor> composite_train_multi_fw(
    const torch::Tensor sigmas,
    ...
    return composite_train_multi_fw_cu(
...
std::vector<torch::Tensor> composite_train_multi_bw(
    const torch::Tensor dL_dopacity,
    const torch::Tensor dL_ddepth,
    const torch::Tensor dL_drend,
    const torch::Tensor dL_dws,
    const torch::Tensor sigmas,
    const torch::Tensor raws,
    const torch::Tensor ws,
    ...
    return composite_train_multi_bw_cu(
                dL_dopacity, dL_ddepth, dL_drend, dL_dws,
                sigmas, raws, ws, deltas, ts, rays_a,
                opacity, depth, rend, opacity_threshold);
}
...
void composite_test_multi_fw(
    ...
    composite_test_multi_fw_cu(
        sigmas, raws, deltas, ts, hits_t, alive_indices,
        T_threshold, N_eff_samples,
        opacity, depth, rend);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m){
    ...
    m.def("composite_train_multi_fw", &composite_train_multi_fw);
    m.def("composite_train_bw", &composite_train_bw);
    m.def("composite_train_multi_bw", &composite_train_multi_bw);
    m.def("composite_test_fw", &composite_test_fw);
    m.def("composite_test_multi_fw", &composite_test_multi_fw);
